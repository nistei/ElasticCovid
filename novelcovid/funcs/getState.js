...
module.exports = async function(params) {
    if (!params.state && !params.sort) {
        let fiftystates = await fetch(`http://localhost:3000/states`).then(r=>r.json());
        ...
        let filteredStates = await fetch(`http://localhost:3000/states`).then(r=>r.json());
        ...
        let filteredState = await fetch(`http://localhost:3000/states`).then(r=>r.json());
